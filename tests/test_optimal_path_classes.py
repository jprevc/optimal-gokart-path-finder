...
class TestGokartGetFResistance:
    ...
    def test_positive_speed_returns_positive_force(self):
        kart = Gokart(mass=100.0, f_grip=500.0, f_motor=200.0, k_drag=0.5)
        # F = k_drag * v^2
...
class TestGokartGetAcceleration:
    ...
    def test_higher_speed_reduces_acceleration(self):
        kart = Gokart(mass=100.0, f_grip=500.0, f_motor=200.0, k_drag=0.5)
...
class TestPathConstruction:
    ...
    def test_valid_input_creates_path(self):
    ...
    def test_invalid_input_too_few_points_raises_value_error(self):
        # Plan: Path should validate at least 2 rows and 2 columns
        pts_one_row = np.array([[1.0, 2.0]])
        with pytest.raises(ValueError):
            Path(pts_one_row)

    def test_invalid_input_single_column_raises_value_error(self):
...
class TestPathPathLength:
    ...
    def test_straight_path_sanity(self):
...
class TestTrackConstruction:
    ...
    def test_even_border_pts_succeeds(self):
        border_pts = np.array(
    ...
    def test_odd_border_pts_count_raises_value_error(self):
        border_pts = np.array(
...
class TestGokartDriveAnimation:
    ...
    def test_init_with_path_sets_attributes(self):
